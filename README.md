...
仓库已包含 `.github/workflows/deploy.yml`。推送到 `main` 分支后，GitHub Actions 会运行 **Build and publish gh-pages branch** 工作流：
...
这说明你正在重跑旧的 `actions/deploy-pages` 工作流，或者 GitHub Pages 仍被设置为 **GitHub Actions** 部署源。请不要点击旧失败记录右上角的 **Re-run jobs**；应当推送/合并最新提交，让新的 **Build and publish gh-pages branch** 工作流重新触发。新的工作流页面里只会看到一个 job：`publish-to-gh-pages`，不会再出现旧的 `build` / `deploy` 两个 job。
